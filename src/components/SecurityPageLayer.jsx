...
import SecurityCreatePageLayer from "../components/SecurityCreatePageLayer.jsx";
import { Input } from "reactstrap";
import axios from "axios";

const SecurityPageLayer = () => {
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/user/getAllUsers/?adminId=${adminId}`
      );
   ...
   const handleUpload = async () => {
      ...
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/user/bulk-upload`,
          formData,
          {
            headers: {
  ...
  return (
    <div className="card radius-16 mt-24">
      <div className="card-header d-flex align-items-center flex-wrap gap-2 justify-content-between">
        <h6 className="mb-2 fw-bold text-lg mb-0">Security </h6>
                  ...
                  {currentDisplayedUsers.length > 0 ? (
                    currentDisplayedUsers.map((user, index) => (
                      <tr key={user._id}>
                        ...
                        {/* <td className="text-center">{user.dept?.name}</td> */}
                        <td className="text-center">
                          <button
      ...
      >
        ...
        <DialogContent>
          <SecurityCreatePageLayer onUserCreated={fetchForms} />
        </DialogContent>
      </Dialog>
...
export default SecurityPageLayer;
